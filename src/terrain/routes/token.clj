...
(def ^:private token-route-description
  (->> ["This service allows users to obtain OAuth tokens for accessing other API endpoints. You must be logged"
        ...
        ""
        "You can use `curl` and `jq`, which is available from [the jq web site](https://stedolan.github.io/jq/), to"
        "obtain an access token from the command line. The easiest way to do this on Unix-like operating systems"
        "is to define an environment variable containing the authorization header:"
        ""
        "```"
        "export AUTH_HEADER=\"Authorization: Bearer $(curl -su username https://de.cyverse.org/terrain/token | jq -r .access_token)\""
        "```"
        "Once you have the authorization header stored in an environment variable, you can include it in calls to"
        "other Terrain endpoints:"
        ""
        "```"
