...
(defschema Worker
  {:id                      WorkerID
   ...
   :activation_expires_on   (describe String "When the worker's activation expires")
   :deactivated_on          (describe String "When the worker was deactivated. Automatically set by the system")
   :activated_on            (describe String "When the worker was activated. Automatically set by the system")
   :getting_work            (describe Boolean "Whether the worker is claiming an event. Set by the worker")
   :getting_work_on         (describe String "When the worker started claiming an event. Automatically set by the system")
   :getting_work_expires_on (describe String "When the worker must stop trying to claim an event. Automatically set by the system")
   :working                 (describe Boolean "Whether the worker is processing a event. Set by the worker")
   :working_on              (describe String "When the worker began processing the event")
...
(defschema Event
  {:id                      EventID
   ...
   :claimed_by              (describe UUID "The UUID of the worker that claimed the event")
   :claim_expires_on        (describe String "The date the worker's claim expires")
   :claimed_on              (describe String "The date the worker claimed the event")
   ...
   :processed_on            (describe String "The date the worker finished processing the event")
